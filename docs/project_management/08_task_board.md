...
| R4-09 | 连续飞行与轨迹对比 | 前端/后端 | Runtime Verified | R2-05/R4-03 | 后端已改为距离参数化平滑轨迹和渐入/渐出视觉修正；前端航向/偏航按最短角插值，避免跨 0° 时反向大幅旋转；测试覆盖最大速度和最大航向变化门禁，3D 和 fallback 地图均展示参考轨迹与融合实际轨迹 |
...
| R8-02 | 检查 OpenCV 环境 | 后端/视觉 | Runtime Verified | v0.4 | `supermap_nav` 已可导入 `cv2 4.13.0`，`ORB_create` 与 `SIFT_create` 均可检测 |
...
| R8-04 | 实现 OpenCV ORB 最小匹配 | 视觉/后端 | Runtime Verified | R8-03 | `/api/vision/localize` 使用 `matcher_mode=opencv_orb` 可输出匹配点数、内点率、置信度、误差半径、估计位姿和失败原因 |
| R8-05 | 接入 SIFT 或可选深度 matcher provider | 视觉 | Todo | R8-03/R8-04 | 若环境支持，提供 `opencv_sift`；LoFTR/LightGlue 仅作为外部接口预留 |
| R8-06 | 生成 v0.5 合成视图 | GIS/视觉 | Runtime Verified | R6 | 已基于正射影像瓦片、航向旋转、视场裁剪和尺度扰动生成 v0.5a 候选视图 |
| R8-07 | DEM/建筑参与合成视图增强 | GIS/视觉 | Runtime Verified | R8-06 | DEM/建筑已进入合成视图 metadata 与解释信息；真实透视投影仍作为 v0.5b 增强 |
| R8-08 | 真实匹配结果进入导航融合 | 后端 | Runtime Verified | R8-04 | `POST /api/navigation/start` 支持 `matcher_mode=opencv_orb`，ORB 结果可写入 `visual_position`、`visual_frame` 和 `fused_position`；失败时降级 v0.4 proxy |
| R8-09 | 前端显示匹配证据 | 前端 | Runtime Verified | R8-04 | 视觉面板已请求 `matcher_mode=opencv_orb`，显示 UAV 图、合成视图、匹配证据图、provider、置信度和误差半径 |
| R8-10 | 定位误差与轨迹偏差报告 | 后端/材料 | Runtime Verified | R8-08 | `/api/reports/{task_id}` 已输出 `navigation_quality`，包含平均置信度、误差半径、融合轨迹偏差、终点误差、provider 统计、回退帧和平滑性 |
| R8-11 | v0.5 验收脚本 | 后端/前端 | Runtime Verified | R8-08/R8-09 | `scripts/check_v05_navigation_gate.ps1` 已一键覆盖 runtime、build、pytest、ORB 证据生成、导航时间线、报告字段、DOM 和截图门禁 |
| R8-12 | v0.5 报告页截图与演示彩排 | 材料/前端/统筹 | Todo | R8-10/R8-11 | 归档报告页截图，完成 3 次“视觉自主导航主线”连续彩排并记录问题 |
